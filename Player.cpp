...
void Player::turn_right(double dphi){
    _dir.phi += dphi;
    if (_dir.phi >= 2. * pi) _dir.phi -= 2. * pi;
}

void Player::turn_left(double dphi){
    _dir.phi -= dphi;
    if (_dir.phi < 0.) _dir.phi += 2. * pi;
}

void Player::turn_up(double dtheta){
    _dir.theta += dtheta;
    if (_dir.theta < (pi - VERTICAL_LIMIT2)) _dir.theta = (pi - VERTICAL_LIMIT2);
}

void Player::turn_down(double dtheta){
    _dir.theta += dtheta;
    if (_dir.theta > (pi - VERTICAL_LIMIT1)) _dir.theta = (pi - VERTICAL_LIMIT1);
}
